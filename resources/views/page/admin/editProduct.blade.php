        ...
        <div class="row rounded-4 shadow-lg d-flex m-auto p-0 w-50 overflow-hidden">
            <div class="col-lg-12 p-5" style="background-color: #F8F9FA;">
                    <div class="card-header">
                        <h1>Update An Product</h1>
                    </div>
                    <form class="card-body" method="POST" action="{{ route('store.editProduct',$Product->ProductID) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="mb-3">
                        ...
                        @error('image')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                        <button type="submit" class="btn btn-danger rounded-pill w-100">Update Product</button>
                        </span>
                    </form>
            </div>
        </div>
